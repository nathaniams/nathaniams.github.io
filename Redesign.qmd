...
Here is another way to visualize the information that blends location while still providing the number of Search and Rescue Incidents for the top parks.
*Note: Sequoia National Park and Kings Canyon National Park were originally combined in the table. However, the NPS has them as separate parks.Therefore the number of incidents were divide between the two parks.
```{r}
#| echo: false
library(leaflet)
library(tidyverse)
...
An unusual error occurred when trying to render from R Studio to Quarto. The above map is missing the data overlay. The map generated without issues in R Studio. Hence the workaround, saving the ggplot as an image and adding to the markup. After revisiting this issue on the Redesign Code page, the issue was discovered in the variable name for the dataframe that was being used in another process.
